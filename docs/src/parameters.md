...
These are all the parameters that are needed for a simulation. For the Simulation configuration files defaults are shown here but the environemt and species parameters have no defaults and have to be supplied.  
...
| `config_dir`                     | ``                 | `String`                     | Path to the configuration folder is taken from [`read_input`](@ref)                                                                                                       |
| `output_dir`                     | `./output/`        | `String`                     | Path to the output folder.                                                                                                                                                |
| `species_dir`                    | `.../species/`     | `String`                     | Path to the species folder. The default assumes a species folder in the folder given to [`read_input`](@ref)                                                              |
| `environment_dir`                | `.../environment/` | `String`                     | Path to the environment folder. Default extends the path given in [`read_input`](@ref)                                                                                    |
