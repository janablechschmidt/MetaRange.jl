## The place where shared functions of eco_loop and eco_loop_optim live to decrease code duplication between them!
...
function GetReproductionModel(reproduction_model::String)
  ...
  elseif reproduction_model == "RickerAllee"
    ...
    throw(ArgumentError(reproduction_model, " is not a supported reproduction model. Currently supported are: Ricker, Beverton, and RickerAllee"))
  end
end
...
function init_species_sim_vars!(species::Array{Species}, LS::Landscape, parameters::Simulation_Parameters, timestep::Int)
  start_time = now()
  for sp in species
    sp.vars.habitat = get_habitat(sp.traits.env_preferences,LS,parameters.env_attribute_mode,timestep)
    sp.habitat[:,:,timestep] = get_habitat(sp.traits.env_preferences,LS,parameters.env_attribute_mode,timestep)
    sp.vars.is_habitat = get_is_habitat(sp.vars.habitat, sp.traits.habitat_cutoff_suitability)
    sp.vars.future_habitat = get_habitat(sp.traits.env_preferences,LS,parameters.env_attribute_mode,timestep+1)
    sp.vars.future_is_habitat = get_is_habitat(sp.vars.future_habitat, sp.traits.habitat_cutoff_suitability)
    sp.vars.biomass = get_biomass(sp.traits.mass, sp.traits.sd_mass, LS.ylength, LS.xlength)
    sp.vars.growrate = get_pop_var(sp.traits.growrate, sp.traits.sd_growrate, exp_growrate, sp.traits.param_const_growrate,
        sp.traits, LS, sp.vars.biomass, parameters.use_metabolic_theory, timestep, E_growrate)
    sp.vars.carry = get_pop_carry(sp.traits, LS, sp.vars.habitat, sp.vars.biomass, parameters.use_metabolic_theory, timestep, E_carry)
    sp.vars.allee = get_pop_var(sp.traits.carry, sp.traits.sd_carry, exp_carry, sp.traits.param_const_carry,
        sp.traits, LS, sp.vars.biomass, parameters.use_metabolic_theory, timestep,E_allee)
    sp.vars.bevmort= get_pop_bevmort(sp.traits, LS, sp.vars.habitat, sp.vars.biomass, parameters.use_metabolic_theory, timestep, E_bevmort)
    sp.vars.occurrences = findall((sp.abundances[:,:,timestep].>0) .& (sp.vars.is_habitat))
  end
  end_time = now()
  @debug("Time needed to simulate the Simulation Variables of all species: ", end_time-start_time)
end

# Get habitat in current timestep returns Array{Float64, 2}
function get_habitat(env_pref::Dict{String, Env_Preferences}, LS::Landscape, attribute_mode::String, t::Int)
  ...
  for i in 1:length(env_keys)
    ...
    habitability_arr[:,:,i] = get_habitat_suit(env_pref[env_keys[i]].upper_limit, env_pref[env_keys[i]].optimum, env_pref[env_keys[i]].lower_limit, conditions)
  end
  habitability = 1
...
function get_pop_var(trait::Float64,sd_trait::Float64, exp_trait::Float64, param_const_trait::Union{Float64,Nothing},
    traits::Traits, LS::Landscape, mass::Array{Float64,2}, use_metabolic_theory::Bool, timestep::Int, E::Float64)
...
function get_pop_bevmort(traits::Traits, LS::Landscape, habitat::Array{Float64,2}, mass::Union{Float64,Array{Float64,2}}, use_metabolic_theory::Bool, timestep::Int, E::Float64)
  bevmort = get_pop_var(traits.bevmort, traits.sd_bevmort, exp_bevmort, traits.param_const_bevmort,
      traits, LS, mass, use_metabolic_theory, timestep,E)
  bevmort = bevmort
...
function Disperse!(species::Vector{Species}, LS::Landscape, groups::NTuple{4, Vector{Chunk}}, timestep::Int64)
  for sp in species
    groupcount = 1
    # initialize offspring
    sp.vars.offspring = zeros(LS.ylength+2*sp.traits.max_dispersal_dist,
...
function Survive!(species::Vector{Species}, DispersalSurvival, t::Int64)
  for sp in species
    occurrences = findall((sp.vars.offspring[1:size(sp.abundances,1),1:size(sp.abundances,2)].>0))
    occurrences = hcat(getindex.(occurrences, 1),getindex.(occurrences, 2))
    sp.abundances[:,:,t+1] = DispersalSurvival(sp.abundances[:,:,t+1],sp.vars.offspring,occurrences,sp.traits.max_dispersal_dist)
    # Survival to the next timestep depending on habitat quality
    sp.abundances[:,:,t+1] = round.(HabitatMortality(sp.abundances[:,:,t+1],sp.vars.future_is_habitat))
...
function disperse_chunk!(species::Species, occurrences::Vector{CartesianIndex{2}}, offspring::Matrix{Float64}, chunk::Chunk, t::Int64)
  for coordinates in occurrences # spatial loop
    ...
    if y >= chunk.y && y < chunk.y+(species.traits.max_dispersal_buffer*2) && x >= chunk.x && x < chunk.x+(species.traits.max_dispersal_buffer*2)
    # Dispersal (Race conditions in SD.Offspring averted by separating landscape into disjunct chunks)
    offspring[y:(y+species.traits.max_dispersal_buffer), x:(x+species.traits.max_dispersal_buffer)] =
    KernelDispersal!(species.abundances[y,x,t+1], offspring[y:(y+species.traits.max_dispersal_buffer),x:(x+species.traits.max_dispersal_buffer)], species.dispersal_kernel)
    #                N,                           Offspring,                                                                                      Kernel
    end
  end
end
...
function GetDisjunctChunkGroups(max_dispersal_buffer::Int64, size::Tuple{Int64, Int64, Int64})
    #separate landsape into 4 disjunct chunk groups
